...
const TableToolbar = ({ filterValue, onFilterChange, filter, search, tab, tabProps, renderTab = true, batchActions, buttonGroup, rowSelection, selectedRows, batchContext, isMobileRender, cardModeToggle }) => {
  ...
  const searchInputNode = showSearch ? (
    <SearchInput
      ...
      {...searchRest}
      style={isMobile ? { ...searchStyle, width: '100%', maxWidth: '100%' } : { ...searchStyle, width: 180 }}
    />
  ) : null;
      ...
      {() => (
        ...
        >
          ...
          {hasValueDisplay ? (
            ...
            >
              <FilterValueDisplay hideLabel value={displayFilterValue} onChange={handleValueDisplayChange} extraExpand={extraExpand} flush={isMobileRender} />
            </div>
          ) : null}
        </div>
      )}
